...
\setstretch{1.5}
{\bf Ans:} Thank you for pointing this out. We are aware of some of the advanced schemes available on the market, but we did not deem that advanced schemes would imply better system performance in our proposed scheme. As the problem is simple and the inherent data do not have particular properties to be exploited (e.g., sparsity) we believe that the proposed schemes have the computational capacity to suitably solve the problem while maintaining a simple structure. However, in order to improve the system performance and for completeness sake, we extended the proposed \ac{nn} architectures to deep learning networks, i.e., by including more hidden layers (see answer 3). 
...
\setstretch{1.5}
{\bf Ans:} Thank you for pointing out this detail. In this paper we do not consider convolutional autoencoders and hence we implemented networks with fully connected layers. We added a more general description of the AE in Sec. \ref{sec:auto} : 
\begin{quote}
...
Fig. \ref{fig:aeArch} has been changed accordingly. Numerical results have been updated including deep \acp{ae}.
...
\setstretch{1.5}
{\bf Ans:} Thank you for pointing out this issue. We have revised all the graphics in the paper and improve their quality to our best.
...
\setstretch{1.5}
{\bf Ans:} As asymptotic results were shown in comparison with the \ac{np} framework described in Sec. \ref{sec:los}, we reviewed the entire Section in order to include more realistic scenarios and show the effect of the different channel parameters on the performance of the \ac{irlv} system. In particular we added in Sec. \ref{sec:los}
\begin{quote}
"\revp{simpleScen}
...
{\bf Ans:} 
\hl{Ruvoletto}
We also added in Sec. \ref{sec:numResOneClass}:
\begin{quote}
"\revp{designAE}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} We added in Sec. \ref{sec:singleAp}:
\begin{quote}
"\revp{building}"
\end{quote}
