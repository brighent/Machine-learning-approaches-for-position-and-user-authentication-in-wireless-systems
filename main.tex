...
{\bf Ans:} 
Thanks for havigh highlighted this issue. We have carefully revised both the \ac{mlp} athe AE and its design choice and we now use common activation functions in the literature. In particular, in Section \ref{sec:numResOneClass} we specify
\begin{quote}
"\revp{designAE}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} Based on your comments we revised Fig. \ref{fig:ceVSmse} and the relative theoretical part in Section \ref{sec:los}. Results show that the implemented system achieves petter performance and the equivalence between hypothesis testing based on the \ac{np} framework and via \ac{ml} is still verified.
