...
\usepackage{booktabs}
\usepackage[width=6.5in, left=2.5cm,height=56em]{geometry}
\makeglossaries
%%% Glossaries/Acronyms
...
\title{Machine Learning \uline{In} In-Region Location Verification \uline{For} Wireless Networks}
...
\revi{rev2lit}{Location verification can be classified into two main sub-problems. The {\it single location verification} problem aim at verifying if a user is in specific point. A solution is obtained by comparing some channel features of the user under test with those of a trusted user that was in the same location in the past. }\revi{rev3cit}{In some works, this approach is used to verify if messages come from the same user, i.e., as a {\it user authentication} mechanism: in \protect{\cite{Baracca-12}} channel features are affected by noise with known  statistics, whereas in \protect{\cite{7398138}} statistics are unknown and a learning strategy is adopted (see also \protect{\cite{7270404}} for a survey).  The {\it \ac{irlv}} aims at verifying if a user is inside a \ac{roi} \cite{Zeng-survey}. }\revi{rev2lit2}{Solutions include distance bounding techniques with rapid exchanges of packets between the verifier and the prover \cite{Brands, singelee2005location}, while in {\cite{song2008secure} the same techniques are applied to vehicular ad-hoc networks}. Other solutions use radio-frequency and ultrasound signals {\cite{Sastry}}, or anchor nodes and increasing transmit power by the sender {\cite{Vora}}. More recently, a delay-based verification technique leveraging geometric properties  has been proposed in {\cite{7145434}}. Some of the proposed techniques partially neglect wireless propagation phenomena (such as shadowing and fading) that corrupt the distance estimates {\cite{Brands,Sastry,Vora}}. Other approaches assume specific channel statistics {\cite{quaglia}} that may be not accurate due to changing environment conditions.} \revi{attack1}{About the attacks to \ac{irlv}, two types have been considered in the literature: a)  sends forged messages to the verification system \mbox{\cite{singelee2005location,song2008secure,Sastry}}  or b) it tampers with the signal power making it coherent with the fake claimed position  \mbox{\cite{Vora,yan2016location,li2010security}}. The first class of attacks targets verification systems which require bits transmission, while countermeasures for the second class are based on channel features like \ac{rss}. In this paper we focus on the second class, as our protocol does not require the decoding of verification messages or nonces.}
...
Two specific security issues are then addressed. Firstly, \uline{it may be difficult to obtain training data from outside the \ac{roi}, that can be a vast area. Therefore, we explore the one-class classification problem under the knowledge of legitimate channel statistics, and we conclude that conventional \ac{ml} solutions based on both the \ac{ae} and one-class \ac{svm} do not coincide with the \ac{glrt}, even for long training.}  \revi{rev14b}{Lastly, we consider  the use of (one-class) classifiers to accelerate the success of a sequence of attacks made from various positions.} Numerical results are provided to support the theoretical results in a realistic wireless network scenario, including path-loss, shadowing, and fading. We show that in a simple scenario a simple \ac{nn} and a relatively small training dataset already provides close-to-optimal performance; then, we assess the one-class \ac{irlv} approach as well as the advanced attack strategy based on \ac{ml}.  
...
\revi{simpleScen2_2}{A similar expression hold for $p(d_0|d_0 \in \mathcal{A}_1)$.} Close-form expressions of the  \ac{llr} in the two scenarios are derived in Appendix \ref{sec:llrDer}.

\revi{llrComp}{We notice however that if multiple \acp{ap} were present or if shdowing and fading correlation where considered it would not have been so easy to obtain a closed form solution for the \acp{llr}, thus the \ac{np} framework could not have been implemented. We thus propose to implement the \ac{irlv} hypothesis testing by \ac{ml} approaches, which do not require the a-priori knowledge of the \acp{pdf}.}
...
\revi{activation}{For the \ac{nn} approach, we used fully connected networks. For the two-class classification problem, the activation function of the input layer is the identity function, while the activation function of the  hidden and output layers is the sigmoid function \protect{\cite[Chapter 6.2.2.2]{goodfellow}}} $(1-e^{-x})^{-1}$. \revi{numResSimplScen3}{All results for \ac{nn} have been reported only for \ac{ce} loss function, as we have shown in Corollary 1 and Theorem 2 that with both loss functions we achieve the same performance of the \ac{np} test.}
...
\appendices
\section{LLRs derivation}\label{sec:llrDer}
...
\end{equation}
\revi{simpleScen3_1}{ from the uniform \ac{ud} distribution we have $p(a|\mathcal{H}_0)=F(\Delta_0,R_{\rm min},R_{\rm in})$, whereas  $p(a|\mathcal{H}_1) = F(\Delta_1,R_{\rm in},R_{\rm out})$.}
...
Moreover, Fig.s  \ref{fig:kf10}\colorbox{red}{controlla label}, and \ref{fig:kf10Oc} now include the comparison with the model in the literature. We have added the following comments on the figures:  
\begin{itemize}
    \item Comment to Fig. \ref{fig:kf10}\colorbox{red}{controlla label}:
    \begin{quote}
...
\begin{enumerate}
    \item  we revised the cases for which we also show the performance of the \ac{np} test, by adding more realistic features and providing close-form expressions for the \acp{llr} used in the \ac{np} test;
    \item we added performance results obtained from experimental data as per your request;
    \item we presented results (without comparison with the \ac{np} test) for channels  including a) path loss with LOS/noLOS characteristics, b) spatially correlated shadowing, and c) fading, i.e., in accordance with the most detailed literature models. 
\end{enumerate}
In particular, about point 1)  we revised the entire Section~\ref{sec:los} in order to include more realistic scenarios and show the effect of different channel parameters on the \ac{irlv} performance. In Section~\ref{sec:los} we now consider the following scenarios:
\begin{quote}
"\revp{simpleScen}"
\end{quote}
Then we derive close-form expressions of the \ac{llr} in both scenarios.
...
\setstretch{1.5}
{\bf Ans:} The proposed \ac{ml} solutions operate with any channel feature, and the comparative analysis on their asymptotic behaviour with respect to \ac{np} test holds irrespective of the input model. Moreover, note that the \ac{ml} approach is model-less by nature, as no assumption is done on the structure/model of the input. In this sense, the details provided for the channel model in Section~II only show the complexity of the \ac{irlv} problem and are used to obtain numerical results in realistic conditions. However, we do not use the specific model of Section~II for the analysis described in the rest of the paper. In order to strengthen the fact that our approach is general and not related to a specific model in the revised manuscript we report performance results obtained by  applying the \ac{ml} \ac{irlv} techniques also on experimental data.

In order to make this point more more clear:
\begin{itemize}
    \item we added the following sentence in Section~ \ref{sec:intro}:
\begin{quote}
"\revp{framework1}"
\end{quote}
\item we added the following sentence in Section~\ref{sec:irlvML}: 
\begin{quote}
"\revp{framework2}"
\end{quote}
\item we added performance results on experimental data in Fig. \ref{fig:Berlinnew} with the following comment:
\begin{quote}
...
Moreover, in Section \ref{sec:singleAp} we added the following sentence:
...
\setstretch{1.5}
{\bf Ans:} Thank you for the suggestion. We added the following sentence in Section \ref{sec:irlvML}
\begin{quote}
"\revp{supervised}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} Parameter $\lambda$ must be chosen to guarantee the desired $P_{\rm FA}$. In order to clarify this point we added the following sentence in Section~\ref{sec:nn}:
\begin{quote}
"\revp{lambdaNN}"
\end{quote}
...
\end{framed}
\setstretch{1.5}
The \ac{nn} can be designed according to many target functions. We considered the widely used \ac{mse} and \ac{ce} functions and shown with two different theorems that with both choices the \ac{nn} achieves performance of the \ac{np} test. 

In order to clarify why both \ac{mse} and \ac{ce} loss functions are considered for \acp{nn} we added the following sentences:
\begin{itemize}
...
\end{framed}
\setstretch{1.5}
We merged Sections D and E on \ac{svm}, as you suggested.
...
\begin{framed}
\noindent {\bf Rev:
Reason for using Eq. (38) [Eq. (\ref{eq:sigmoid}) in the revised paper] as activation function for the output layer is missing, especially since it is not a common choice for an activation function. Motivate the design choices!}
\end{framed}
\setstretch{1.5}
...
\revp{literature_3}"
\end{quote}
Moreover, Fig.s  \ref{fig:kf10}, and \ref{fig:kf10Oc} now include the comparison with the literature model. We have added the following comments on the figures:  
\begin{itemize}
