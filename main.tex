...
\revi{attack1}{In the location verification literature there are two classes of threats, either the attacker sends forged messages to the verification system \cite{singelee2005location,song2008secure,Sastry} or he tampers with the signal power making it coherent with the fake claimed position  \cite{Vora,yan2016location,li2010security}. The first class of attacks targets verification systems which require bits transmission, while countermeasures for the second class are based on channel features like \ac{rss}. In this paper we focus on the second class, as our protocol does not require the decoding of verification messages or nonces.}
