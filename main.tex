...
\end{equation}
where $\zeta(\rho )$ denotes the angle of the circular sector located at distance $\rho$ intersecting area $\mathcal{A}_0$. By taking the derivative of (\ref{eq:cdf}) with respect to $R_0$, we obtain the \ac{pdf} of $R_0$ given that the \ac{ue} is located in $\mathcal{A}_0$:
\begin{equation}\label{eq: num}
    p_{R|\mathcal{A}_0}(R_0 |\mathcal{A}_0) = \frac{1}{|\mathcal{A}_0|}R_0 \zeta(R_0).
\end{equation}
Following the same reasoning, if we consider that the length of the arc of circle with radius $R_0$ located in $\mathcal{A}_1$ is $2\pi - \zeta(R_0)$, we obtain the \ac{pdf} of  $R$ given that the \ac{ue} is located in $\mathcal{A}_1$ as
...
According to the \ac{mse} design criterion \cite{goodfellow}, the \ac{mlp} parameters are estimated in order to minimize the \ac{mse} during the training phase:
\begin{equation}
...
\end{equation}
The function (\ref{testfunNN}) imposes a threshold $\lambda$ on $g_0(\bm{a})$ and reorganizing terms we obtain $f(\bm{a}) = -1$ when
\begin{equation}
...
\end{equation}
When training is performed with a \ac{ce} loss function, the output of the \ac{mlp} is the minimum \ac{mse} approximation of the probability $\mathbb{P}(\mathcal{H}_0|\bm{a}^{(i)})$ of being in hypothesis $\mathcal{H}_0$ given the attenuation vector $\bm{a}$ \cite{Bishop2006}, i.e.,
\begin{equation}
...
While the feature-space transformation function is typically fixed \cite{goodfellow}, vector $\bm{w}$ must be properly learned from the data to perform the desired hypothesis testing. 
...
In this section, we present the numerical results obtained using the proposed \ac{irlv} and attack solutions for the scenario described in Section II. (\textbf{Qui inserirei un breve riassunto dell'experimental setup, non solo per le NN. Giorgio}) In particular, for the \ac{nn} for two-class classification has one hidden layer. The activation function for the input layer is the identity function, for the hidden layer is the sigmoid function
\begin{equation}
...
\paragraph*{\ac{los} Model} We consider an overall circular area $\mathcal{A}$ with radius $R_{\rm out} = 40$~meters, a square authentic area $\mathcal{A}_0$ of $L = H= 25$~meters located inside $\mathcal{A}$, with upper left corner at a distance of $R_{\rm min} = 4$~meters from the center of $\mathcal{A}$. In the simplified scenario of Section \ref{sec:los}, we have a closed-form expression of the \ac{np} test function. For the \ac{ml} approaches, we consider $S=10^6$ training points and various numbers $N_h$ of the hidden layer neurons.
...
Fig. \ref{fig:ceVSmse} shows the \ac{fa} versus (vs.) the \ac{md} probabilities -- the so-called \ac{roc} --  obtained with various \ac{irlv} techniques. In particular, we compare the \ac{np} test function with \ac{ls}-\ac{svm} and \ac{mlp} designed according both \ac{mse} and CE criteria, denoted as \ac{nn}-\ac{mse} and \ac{nn}-\ac{ce} respectively. 

We first note that both \ac{nn}-\ac{ce} and \ac{nn}-\ac{mse} obtain the same performance for the same number of neurons $N_h$ as expected since they both are equivalent to \ac{np} testing in the asymptotic regime. When compared to the \ac{np} approach, we note that the performance of the \ac{nn}-based \ac{irlv} approaches the optimal \ac{np} test as the number of hidden layer neurons grows, and the equivalence is reached already with $N_h=3$ neurons. Moreover, \ac{ls}-\ac{svm} attains the same performance of the \ac{nn} solution, confirming that \ac{svm} is optimal in the \ac{np} sense.

In general, the achieved probabilities of false alarms and mis-detection $(P_{\rm FA}, P_{\rm MD})$ show quite high values: this is due to the fact that a single \ac{ap} is used for \ac{irlv}, and there are many points both outside and inside the \ac{roi} that are at the same distance from the \ac{ap}. Since the attenuation is a function only of this distance, there is a critical ambiguity in the attenuation and the hypothesis testing is in many cases unreliable.
...
We now focus on the one-class \ac{irlv} solutions, where the training points come only from the \ac{roi} $\mathcal A_0$.  We first consider a scenario with path-loss and shadowing without fading.
...
\noindent {\bf Rev:
The machine learning part looks trivial and too classic. Much more advance schemes are available everyday on the market.}
...
\noindent {\bf Rev:
Neural networks in Figure 2 are fully connected and kind of shallow.}
...
In the numerical simulations a comparison with state of the art algorithms should be included.}
...
\noindent {\bf Rev:
Theorem 1 is basically a Corollary from [18].}
...
\noindent {\bf Rev:
In the assessment of Fig 3, the advantage of NN versus N-P should be clarified, too, since they perform equally (bad).}
...
\noindent {\bf Rev:
The results in terms of the miss detection rate and the false alarm rate in Fig. 3 are not practical. Please explain.}
