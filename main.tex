...
\subsection{\Acl{mmse} Approach}
\revi{literature_1}{With known channel statistics we can tackle \ac{irlv} by solving \eqref{eq:los} and \eqref{eq:nlos} for $L(\bm{x}_{\rm UE},\bm{x}_{\rm AP}^{(n)})$ and then find the user position using a \ac{mmse} approach, like in \cite{li2010security}. This consists in solving the following optimization problem
}
	\begin{align}
 \hat{\bm x}_{\rm UE} =  \underset{\bm x}{\arg \min} \sum_{i=1}^{N_{\rm AP}} \left(L(\bm{x},\bm{x}_{\rm AP}^{(n)}) - L(\bm{x}_{\rm UE},\bm{x}_{\rm AP}^{(n)}) \right)^2,
\end{align}
\revi{literature_2}{where $\hat{\bm x}_{\rm UE}$ is the estimated user position. Then, we check whether $\hat{\bm x}_{\rm UE}$ lies inside or outside $\mathcal{A}_0$. More specifically, since the estimation is affected by noise, we build a \ac{roc} by defining $\mathcal{A}_0 ^ {(\theta)}$ as scaled versions of $\mathcal{A}_0$, such that $|\mathcal{A}_0 ^ {(\theta)}| \triangleq \theta|\mathcal{A}_0|$, with $\theta \in \mathbb{R}$.
The \ac{mmse} approach decision function becomes then
}

\begin{equation}
\hat{\mathcal{H}}_{\rm MMSE} = 
\begin{cases}
\mathcal{H}_0 \quad \text{if } \hat{\bm x}_{\rm UE} \in \mathcal{A}_0 ^ {(\theta)} \\
\mathcal{H}_1 \quad {\rm otherwise}.
\end{cases}
\end{equation}

\revi{literature_3}{Note that here the shadowing acts as a non-Gaussian noise, since to retrieve $L(\bm{x}_{\rm UE},\bm{x}_{\rm AP}^{(n)})$ we go from the logarithmic domain (in which shadowing is Gaussian distributed) to the linear domain.
Moreover, this approach relies not only on the knowledge of the path-loss models \eqref{eq:los} \eqref{eq:nlos}, but also on the ability to distinguish between \ac{los} and non-\ac{los}.
In the next Sections we see how the proposed \ac{irlv} system by \ac{ml} achieves better performance under no previous knowledge of any channel statistics and with fading, which was not considered in \cite{li2010security}.}
...
In order to make this point more clear we have updated the Introduction, by extended the literature survey, and adding the following sentence:
\begin{quote}
    "\revp{rev11a}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} \hl{CAMBIARE LA FIG. 3!!!} We thank the reviewer for his/her comment. We have modified Fig. \ref{fig:aeArch} including the possibility of not having some connections among neurons. We also added the following more extensive description of the AE:
\begin{quote}
    "\revp{deep ae}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} We thank the reviewer for his/her comment. We have revised the abstract to avoid misunderstanding about the attacks. It now reads as follows:
\begin{quote}
    "\revp{rev14}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} As asymptotic results were shown in comparison with the \ac{np} framework described in Section \ref{sec:los}, we revised the entire Section in order to include more realistic scenarios and show the effect of the different channel parameters on the \ac{irlv} performance. In particular we added in Sec. \ref{sec:los}
\begin{quote}
"\revp{simpleScen}
