...
% \subsection{Distribution of the likelihood functions}
% The numerator and denominator terms in (\ref{eq:lr}) can be obtained by integration over the areas $\mathcal{A}_0$ and $\mathcal{A}_1$ respectively. In fact, consider a \ac{ue} located in area $\mathcal{A}_0$ with coordinates $(X_u,Y_u)$. Consider for instance the numerator term. The probability of \ac{rss} assuming values $R(X_u,Y_u)$ from a transmission of a \ac{ue} located in $(X_u,Y_u) \in \mathcal{A}_0$ can be written as
% \begin{equation}
%   p(R(X_u,Y_u),(X_u,Y_u) \in \mathcal{A}_0)=p(R(X_u,Y_u)|(X_u,Y_u) \in \mathcal{A}_0)p((X_u,Y_u)\in \mathcal{A}_0). 
% \end{equation}
% By integration over all the possible positions $\mathcal{A}_x$ and $\mathcal{A}_y$ we obtain the distribution of the \ac{rss} given that the \ac{ue} is transmitting from area $\mathcal{H}_0$
% \begin{equation}\label{eq:intH0}
%     p(R|\mathcal{H}_0) = \int_{\mathcal{A}_{x,0}}\int_{\mathcal{A}_{y,0}} p(R(a,b)|(a,b))P(a,b) \, da \, db,
% \end{equation}
% where $p(a,b)$ is the probability that a \ac{ue} location is $(a,b)$.

% In the following we assume that \ac{ue}s are uniformly located in the area, so that 
% \begin{equation}\label{eq:uni}
%     p\big((a,b) \in \mathcal{A}_0\big) = \frac{|\mathcal{A}_0|}{|\mathcal{A}_1|},
% \end{equation}
% and
% \begin{equation}
%     p\big((a,b) \in \mathcal{A}_1\big) = \frac{|\mathcal{A}_1|- |\mathcal{A}_0|}{|\mathcal{A}_1|},
% \end{equation}
% By exploiting the distribution of (\ref{eq:rss}) and considering (\ref{eq:uni}), (\ref{eq:intH0}) can be rewritten as
% \begin{equation}\label{eq:p_h0}
%     p(R|\mathcal{H}_0)= \frac{|\mathcal{A}_0|}{|\mathcal{A}_1|}\int_{\mathcal{A}_x}\int_{\mathcal{A}_y} \frac{1}{2 \pi \sigma_x^2} exp \bigg ( -\frac{1}{2}\frac{\big ( R-P_l(a,b)\big )^2}{\sigma_x^2} \bigg ) \, da \, db.
% \end{equation}
% With the same reasoning the denominator of (\ref{eq:lr}) can be obtained as
% \begin{equation}\label{eq:p_h1}
%   p(R|\mathcal{H}_1)= \frac{|\mathcal{A}_1|-|\mathcal{A}_0|}{|\mathcal{A}_1|}\int_{\mathcal{A}_{x,1}}\int_{\mathcal{A}_{y,1}} \frac{1}{2 \pi \sigma_x^2} exp \bigg ( -\frac{1}{2}\frac{\big ( R-P_l(m,n)\big )^2}{\sigma_x^2} \bigg ) \, dm \, dn.
% \end{equation}
