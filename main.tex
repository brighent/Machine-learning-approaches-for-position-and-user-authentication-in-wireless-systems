...
\begin{equation}
	\begin{aligned}
		&\underset{\mathbf{w},b}{\text{min}} \Exp{ \left( e(\mathbf{a},\mathbf{w},b) - \left(-\frac{1}{p(\mathbf{a}|\mathcal{H}_0)}\right)\right) ^2} = \\
...
\end{equation}
since we have shown that at the optimum $\rho^*>0$.
...
\label{alg:rnnGrad}
 ...
 \KwData{ trained attack \ac{rnn}, $\delta$}
 \KwResult{successful attack vector }
 

 \Repeat{attack is successful}{
        generate a block $A$ of random vectors \;
        select $\bm{a}_{\rm max} = \underset{\bm{a} \in \bm{A}}{\min}\epsilon(\bm{a})$ \;
        generate attack $\mathbf{a}_f$ via (\ref{eq: rnn attack})\;
        perform the attack \;
        \If{
        	attack is not succesful}
        	{update training of the \ac{rnn} with $\mathbf{a}_f$\;}
      }
