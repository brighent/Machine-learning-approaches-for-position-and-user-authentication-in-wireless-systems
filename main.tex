...
\revi{framework1}{The more realistic realistic channel model we take into account is instrumental to show that exact likelihood ratio computation is not feasible while resorting to \ac{ml} techniques makes \ac{irlv} possible. This suggests that the same idea can be applied in different contexts. Namely one could use different channel features, or exploit other sensor measurements depending on the application. In this paper we focus on wireless communication scenario in which standard communication sensors are available, but we believe that the extension to a more general framework is straightforward.}
...
\revi{framework2}{We stress the fact that now the channel model of Section \ref{sec:chMod} has the purpose of simulating a realistic communication scenario, while the analysis that follows is general, as no specific channel statistic knowledge assumed.}
