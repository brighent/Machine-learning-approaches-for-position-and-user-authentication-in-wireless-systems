...
\usepackage[backend=bibtex,style=ieee,sorting=none,style=numeric-comp]{biblatex}
...
\end{enumerate}
\revi{lit2}{In particular, about the first point we design an \ac{irlv} solution able to operate in a scenario with channel impairments such us shadowing and fading, which is new in the location verification literature. Indeed, in {\cite{Vora}} \ac{rss} measurements are explicitly treated interchangeably as distance measures neglecting all shadowing and fading phenomena. Similarly, in {\cite{Sastry}} it is generically assumed that agents are able to communicate and exchange packets without errors, as in all protocols requiring nonces transmission {\cite{singelee2005location,song2008secure}}. In {\cite{yan2016location}} and {\cite{li2010security}} shadowing is taken into account, even though not fading. However, in {\cite{yan2016location}} shadowing is relative to a single user position, allowing the authors to derive the channel statistic and in {\cite{li2010security}} authors assume the knowledge of the path-loss model. Both these hypotheses are not required by our \ac{irlv} system. Moreover, in this paper we focus on \ac{irlv}, which is different from verifying a claimed specific location {\cite{singelee2005location} \cite{li2010security}} or authenticating a user with its location {\cite{yan2016location}}}.
...
\label{sec:comp}
\revi{comp1}{In this section, we briefly review the computational cost for i) training each model and ii) make a prediction on a new data point. Let $\eta$ be the number of epochs (how many times each training point will be used) of a \ac{nn} and $N_{SV}$ the number of support vectors of a \ac{svm}.}
...
\uline{In this section, we present the performance of the proposed \ac{irlv} obtained from both experimental data and the channel models of Section II. We consider a unitary transmitting power for each user and a carrier frequency of $f_0=2.12$~GHz, apart for the experimental results {\color{red}non chiaro}, using the \ac{gsm} frequency $f_0 =900$~MHz. When spatial correlation of shadowing is considered, we consider a correlation distance of $d_c = 75$~m according to the model of Section II.} 

\uline{The training points for the classification tasks are taken uniformly over the area $\mathcal A$ ($\mathcal A_0$).}
 
\revi{activation}{For the \ac{nn} approach, we used fully-connected networks, and, for the two-class classification problem, the activation function of the input layer is the identity function, while the activation function of the  hidden and output layers is the sigmoid function}
\begin{equation}\label{eq:sigmoid}
\psi^{(\ell)}(x) = \frac{1}{1-e^{-x}}, \quad \ell = 1, \ldots, Q.
\end{equation}
\revi{numResSimplScen3}{All results for \ac{nn} have been reported only for \ac{ce} loss function, as we have shown in Corollary 1 and Theorem 2 that with both loss functions we achieve the same performance of the \ac{np} test.}.
...
\uline{We consider the environment of Fig. \ref{fig:mBS} with $N_{\rm AP}=10$ \acp{ap} used for \ac{irlv}, \ie $\rm AP_i$ with $i = 2,\dots,11$. The channel model includes \ac{los} and no-\ac{los} path-loss, spatially-correlated shadowing ($\sigma_{\rm s, Db} = 8$), and fading, as described in Section II. We use a \ac{nn} with $L=3$ hidden layers, each layer having $N_h = 100$ neurons.}  
...
\colorbox{red}{METTERE I NUMERI E LA DESCRIZIONE}\revi{revnewarea}{We have also considered a different position of the \ac{roi}, with $d_1 = 100$~m, $d_2 = 255$~m and $\beta_1 = \beta_2 = 150$~m, i.e., the \ac{roi}  is still positioned south-west, but it also includes the street crossing and $\rm AP_8$ (see Fig. \ref{fig:mBS}). Path-loss and shadowing parameters are the same of Fig. \ref{fig:kf1}. Fig. \ref{fig:kf1_newArea} shows the resulting \ac{roc}, still obtained by averaging the \ac{md} probabilities over the shadowing maps. The presence of the street, inside the \ac{roi}, with its  \ac{los} path-loss, turns out to facilitate \ac{irlv}, resulting in lower \ac{fa} and \ac{md} probabilities.}


\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\columnwidth]{res_training_10BS_2Class.eps}
    \caption{\ac{roc} of \ac{irlv} methods for different values of training-set size $S$. Environment of Fig. \ref{fig:mBS}, with  $N_{\rm AP}=10$, $d_1 = 50$~m, $d_2 = 50$ and $\beta_1 = \beta_2 = 150$~m , $\sigma_{s,{\rm dB}} = 8$~dB.}
    \label{fig:kf1}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{irlv} methods for different values of training-set size $S$. Environment of Fig. \ref{fig:mBS}, with  $N_{\rm AP}=10$, $d_1 = 100$~m , $d_2 = 225$~m and $\beta_1 = \beta_2 = 150$, $\sigma_{s,{\rm dB}} = 8$~dB.}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{irlv} methods for different averages of fading. Environment of Fig. \ref{fig:mBS}, with  $N_{\rm AP}=5$, $d_1 = 50 $~m, $d_2 = 50$~m and $\beta_1 = \beta_2 = 150$~m, $\sigma_{s,{\rm dB}} = 8$~dB.}
    \label{fig:kf10-5}
\end{figure}
...
\begin{enumerate}
    \item  we have revised the cases in which we show also the performance of \ac{np} test, adding some more realistic details and providing close-form expressions for the \acp{llr};
    \item we added performance results obtained from experimental data as per your request;
    \item we present results for channel model including a) path loss with LOS/noLOS characteristics, b) spatially correlated shadowing, and c) fading, i.e., in accordance with the most detailed channel models used also in standardization processes (such as the 3GPP standards). 
\end{enumerate}
In particular, about point 1)  we revised the entire Section \ref{sec:los} in order to include more realistic scenarios and show the effect of the different channel parameters on the \ac{irlv} performance. In Section \ref{sec:los} we consider the following scenarios
\begin{quote}
"\revp{simpleScen}"
\end{quote}
Then we derive close-form expressions of the \ac{llr} in both scenarios.

In the Numerical Result section performance results are reported for the new scenarios, confirming that \acp{nn} and \ac{svm} match the performance of the \ac{np} test. In particular we write:
\begin{quote}
...
\setstretch{1.5}
{\bf Ans:} The proposed solutions operate with any channel feature, and the analysis about the asymptotic behaviour of \ac{ml} techniques with respect to \ac{np} test hold irrespective of the model of the input. Moreover, note that the \ac{ml} approach is model-less by nature, as no assumption is done on the structure/model of the input. In this sense, the details provided for the channel model in Section II only show the complexity of the \ac{irlv} problem and are used to obtain exemplary numerical results. However, we do not use the specific model of Section II in the rest of the paper. In order to strengthen the fact that our approach is general and not related to a specific model we have applied the \ac{ml} \ac{irlv} technique also on experimental data.

In order to make this more more clear:
\begin{itemize}
...
\setstretch{1.5}
{\bf Ans:} Thank you for pointing out this. We added in Section \ref{sec:auth}
\begin{quote}
"\revp{lambda}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} Since Theorems 1 and 2 hold, the hypothesis testing implemented by the \ac{nn} classifier is the same implemented by the \ac{np} framework. Therefore the parameter $\lambda$ does not need to be optimize, but instead guarantees the minimum $P_{\rm MD}$ for the chosen $P_{\rm FA}$. In order to clarify this points we added in Section \ref{sec:nn}:
\begin{quote}
"\revp{lambdaNN}"
\end{quote}
...
The choice of this new setting is motivated by the fact that, as reported in Section \ref{sec:OneClass}:
\begin{quote}
"\revp{revOC}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} As \acp{ae} are well known architectures, the in-depth description of the meaning of the low dimensional layer as well as its derivation is beyond the scope of this paper, as well it is the choice of the linear activation function at the output layer (usual choice according to \cite[Chapter 14]{goodfellow}). We added in Section \ref{sec:auto} a reference to \cite[Chapter 14]{goodfellow}, for a more accurate description of the characteristics of \acp{ae}.
...
{\bf Ans:} 
Thanks for havigh highlighted this issue. We have carefully revised both \ac{mlp} and AE design choices and we now use common activation functions in the literature. In particular, in Section \ref{sec:numRes} we added
...
\setstretch{1.5}
{\bf Ans:} In the revised manuscript both the feedforward \acp{nn} and the \ac{ae} hev been extended to include multiple layers in order to improve the system performance as you suggested. Results are reported in Section \ref{sec:numRes}.
...
\setstretch{1.5}
{\bf Ans:} Thank you for the suggestion. We added in Section \ref{sec:los} Fig. \ref{fig:simpScen} to illustrate the scenario where the \ac{np} framework has been applied and for which Section \ref{sec:singleAp} reports the numerical results.
...
\setstretch{1.5}
{\bf Ans:} We added in Section \ref{sec:los}:
\begin{quote}
"\revp{llrComp}"
\end{quote}
...
Moreover, in the revised version of the paper we consider also another position of the \ac{roi}, with $d_1 = 100$~m, $d_2 = 255$~m and $\beta_1 = \beta_2 = 150$~m, i.e., the \ac{roi}  is still positioned south-west, but it also includes the street crossing and $\rm AP_8$ (see Fig. \ref{fig:mBS}). . We added Fig. \ref{fig:kf1_newArea} that has been commented as follows:
\begin{quote}
"\revp{revnewarea}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} Based on your comments we revised Fig. \ref{fig:ceVSnp} and the relative theoretical part in Section \ref{sec:los}. Results show that the implemented system achieves petter performance and the equivalence between hypothesis testing based on the \ac{np} framework and via \ac{ml} is still verified.
