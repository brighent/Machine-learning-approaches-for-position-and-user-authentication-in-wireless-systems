...
\begin{abstract}
\uline{\Ac{irlv} aims at verifying whether a user is inside a \underline{region of interest (ROI)}, and in wireless networks it can exploit the features of the channel between the user to be verified and a set of trusted access points. When channel statistics are known, we can resort to the \ac{np} test. In practical scenarios these statistics are not available and we resort to  \ac{ml} solutions based on either \acp{nn} or \acp{svm} to learn channel features characterizing the ROI. We show that \ac{ml} approaches are \ac{np}-optimal for sufficiently complex machines and long enough training with typical loss functions.} \revi{rev14}{When in the training phase channel feature estimates are available  only from the \ac{roi}, we resort to one-class classifiers, namely auto-encoders and one-class \acp{svm}. However,  we prove that they are not equivalent to the  \acf{glrt}, that typically replaces \ac{np} when statistics under only one hypothesis are available.} Numerical results are provided to support the results in  realistic \uline{wireless networks}, with channel model including path-loss, shadowing, and fading.
\end{abstract}
...
\uline{Location information without verification gives ample opportunities to attack a service granting system (with  applications in sensor networks \mbox{\cite{Zeng-survey, 8376254, wei2013}}, the Internet of Things \cite{7903611}, and geo-specific encryption \cite{quaglia}), since the location information can be easily manipulated either by tampering the hardware/software reporting the location or by spoofing the \ac{gnss} signal outside the user device. In this context, location verification systems aim at verifying the position of mobile devices in a communication network.  In order to verify the location, the features of the wireless channel over which communication occurs can be exploited. An example is given by {\cite{li2010security}}, where \ac{rss} is used to estimate the distance between the user and other network nodes.}

\revi{rev2lit}{Location verification can be classified into two main sub-problems. The \emph{ single} \emph{location} \emph{verification} problem aims at verifying if a user is in a specific point. A solution is obtained by comparing some channel features of the user under test with those of a trusted user that was in the same location in the past. }\revi{rev3cit}{In some works, this approach is used to verify if messages come from the same user, i.e., as a {\it user authentication} mechanism: in \protect{\cite{Baracca-12}} channel features are affected by noise with known  statistics, whereas in \protect{\cite{7398138}} statistics are unknown and a learning strategy is adopted (see also \protect{\cite{7270404}} for a survey).  The {\it \ac{irlv}} aims at verifying if a user is inside a \ac{roi} \cite{Zeng-survey}. }\revi{rev2lit2}{Solutions include distance bounding techniques with rapid exchanges of packets between the verifier and the prover \cite{Brands, singelee2005location}, while in {\cite{song2008secure} the same techniques are applied to vehicular ad-hoc networks}. Other solutions use radio-frequency and ultrasound signals {\cite{Sastry}}, or anchor nodes and increasing transmit power by the sender {\cite{Vora}}. More recently, a delay-based verification technique leveraging geometric properties  has been proposed in {\cite{7145434}}. Some of the proposed techniques partially neglect wireless propagation phenomena (such as shadowing and fading) that corrupt the distance estimates {\cite{Brands,Sastry,Vora}}. Other approaches assume specific channel statistics {\cite{quaglia}} that may be not accurate due to changing environment conditions.} 
\revi{attack1}{About the attacks to \ac{irlv}, two types have been considered in the literature: a)  sends forged messages to the verification system \mbox{\cite{singelee2005location,song2008secure,Sastry}}  or b) it tampers with the signal power making it coherent with the fake claimed position  \mbox{\cite{Vora,yan2016location,li2010security}}. The first class of attacks targets verification systems which require bits transmission, while countermeasures for the second class are based on channel features like \ac{rss}. In this paper we focus on the second class, as our protocol does not require the decoding of verification messages or nonces.}

Focusing on \ac{irlv}, if the statistics of the channels to devices both inside and outside the \ac{roi} are known to the network infrastructure, the optimal solution is given by the \ac{np} theorem {\cite{Cover-book}} that provides the most powerful test for a given significance level. When  the channel statistics are not available, a two-step solution would be to a) estimate the channel statistics and b) apply the \ac{np} theorem on the estimated statistics. However, as we also confirm in this paper, this approach may not be accurate. Alternatively, \ac{ml} techniques can be used. For example, in \cite{xiao-2018} the single location verification problem is solved without assumptions  on the channel model by applying logistic regression. In \cite{tian2015robust}, the objective is to determine the position of a user inside a building and a multi-class classification problem is solved via \ac{svm}. Nevertheless, neither \cite{xiao-2018}  nor \cite{tian2015robust} compare the performance of their \ac{ml} approaches with the theoretical optimum \ac{np} criterion.

In this paper, we remove the channel knowledge assumption and study two \ac{ml} solutions for \ac{irlv}, based on \acp{nn} and \ac{svm}. About the loss functions, we investigate the \ac{ce} and the \ac{mse}  for  \ac{mlp}, and  the \ac{ls} for \ac{svm}. We show that these  approaches are \ac{np}-optimal for sufficiently complex machines and sufficiently large training data.  \revi{rev11a}{The obtained asymptotic results are applicable also to elaborate ML solution, such as deep learning NNs, that can still be seen as parametric functions, although more complex than shallow NNs.}

%Two specific security issues are then addressed. 
\uline{Since it may be difficult to obtain training data from outside the \ac{roi}, that can be a vast area, we explore the one-class classification problem under the knowledge of legitimate channel statistics, and we conclude that conventional \ac{ml} solutions based on both the \ac{ae} and one-class \ac{svm} do not coincide with the \ac{glrt}, even for long training.}  
...
Numerical results are provided to support the theoretical results in a realistic wireless network scenario, including path-loss, shadowing, and fading. We show that in a simple scenario a simple \ac{nn} and a relatively small training dataset already provides optimal performance; then, we assess the one-class \ac{irlv} approach.

\uline{The contributions of this paper are summarized hereby:}
\begin{enumerate}
    \item \uline{we propose physical-layer \ac{irlv} solutions based on \ac{ml} techniques: the proposed solutions are suitable to operate with inaccurate estimates, even when their statistics are not known, thus being  model-less;}
    \item \uline{we show that, in asymptotic training and complexity conditions, \ac{nn} and \ac{lssvm} achieve the \ac{fa}-\ac{md} probabilities of the \ac{np} test, which is most powerful for a given significance level;}
\end{enumerate}
\revi{lit2}{About point 1, dealing with shadowing and fading when performing \ac{irlv} has not been much considered in the literature: for example, in {\cite{Vora}} \ac{rss} estimates are  assumed to be error-free distance, while in  {\cite{Sastry}} agents are assumed  to communicate over an  error-free channel (see also most distance-bounding protocols  {\cite{singelee2005location,song2008secure}}). In {\cite{yan2016location}} and {\cite{li2010security}} shadowing is taken into account, while fading is neglected, and  channel statistics are assumed to be known. Both these hypotheses are not required by our \ac{irlv} \ac{ml} algorithms.} \revi{framework1}{Indeed, we will also consider an accurate wireless channel model (in Section II), but only in order to explain the complexity of using literature techniques (including the \ac{np} test), thus justifying the use of  \ac{ml}. Still,  our solution and theoretical results can  be applied on any channel statistics and different channel  (see \cite{Zeng-survey} for a survey), even including  measurements from external sensors.}


\uline{In the rest of the paper, Section~II introduces the system model for the \ac{irlv} problem, with an example of wireless channel, and provide reference \ac{irlv} techniques. Section~III describes the proposed \ac{ml} solutions, and obtain the theoretical results on their asymptotic performance. In Section~IV, we propose the one-class classification approaches. Numerical results are shown and discussed in Section~V, before conclusions are outlined in Section~VII.}
...
We consider a wireless network  with\footnote{The following notation is used throughout the paper: bold lowercase letters refer to vectors, whereas bold uppercase letters refer to matrices, $\mathbb{E}[\cdot]$ and $\mathbb P[\cdot]$ denote the expectation and probability operators, respectively, $(\cdot)^T$ denotes the transpose operator, and $\ln x$, $\log_{10} x$ denote the natural-base and base-10 logarithms, respectively.} $N_{\rm AP}$ \acp{ap} covering the area $\mathcal{A}$ over a plane. We propose a \ac{irlv} system to determine if a \ac{ue} is transmitting from within an {\em authorized} \ac{roi} $\mathcal{A}_0$ inside  $\mathcal{A}$, while its complementary region is $\mathcal{A}_1=\mathcal{A} \setminus \mathcal{A}_0$. The authentication process exploits the location dependency of the features of the channel between the \ac{ue} and the \acp{ap}. \revi{revPHASE}{As an example, we consider as feature the channel power attenuation (of a narrowband transmission), similarly to other works in the literature \protect{\cite{li2010security,yan2016location,Vora}}. Indeed, other features can be exploited, such as the phase or the wideband impulse response: ours solution readily apply also to these cases, as we do not make special assumptions on the channel model for their design and analysis.}

We assume that the \ac{ue} transmits a pilot signal with fixed power, known at the \acp{ap}, from which the \acp{ap} can measure the received power and estimate the channel attenuation. We assume that the attenuation estimation is perfect, i.e., not affected by noise or interference: this can be achieved by using a sufficiently long pilot signal.



\subsection{Channel Model}\label{sec:chMod}

We now describe a widely adopted wireless channel model to clarify the challenge faced by an \ac{irlv} based on attenuation estimate. \revi{WiFi2}{In particular we consider a general channel model that covers a large frequency range from hundreds of MHz to 6~GHz, suitable for wireless local area networks (WLANs) and IoT, where \ac{irlv} is typically applied.} Let $a^{(n)}$ be the attenuation incurred over the channel between the \ac{ue} and \ac{ap} $n$. The attenuation coefficient includes the effects of path-loss, shadowing and fading. In particular, by assuming Rayleigh model for fading we have $g = (\sqrt{a^{(n)}})^{-1} \sim {\mathcal N}\left(0,\sigma_{g,n}^2\right)$, where ${\mathcal N}(m,\sigma^2)$ denotes a Gaussian random variable with mean $m$ and variance $\sigma^2$, $(\sigma_{a,n}^2)_{\rm dB} =  -10\log_{10}\sigma_{g,n}^2={P_{\rm PL}^{(n)}} + s$ accounts for the path-loss and shadowing components, $P_{\rm PL}^{(n)}$ is the path-loss coefficient in dB, and $s \sim \mathcal{N}(0,\sigma_{s,{\rm dB}}^2)$ is the shadowing component.   \uline{Shadowing components of two \acp{ue}  at  positions $\bm{x}_i$ and $\bm{x}_j$   have correlation $\sigma_{s,{\rm dB}}^2e^{-\frac{L(\bm{x}_i,\bm{x}_j)}{d_c}}$, where $d_c$ is the shadowing decorrelation distance \underline{\cite[Section 2.7]{goldsmith2005}}.} 
...
\begin{equation}\label{eq:nlos}
    P_{{\rm PL}, {\rm NLOS}}\left(L(\bm{x}_{\rm UD},\bm{x}_{\rm AP}^{(n)})\right) = 40\log_{10}\left (\frac{L(\bm{x}_{\rm UD},\bm{x}_{\rm AP}^{(n)})}{10^3}\right ) + 21\log_{10}\left(\frac{f}{10^6}\right) + 80.
\end{equation}

Path-loss and shadowing components (thus $\sigma_{a,n}^2$) are assumed to be time-invariant, while the fading (thus attenuation $a^{(n)}$) is independent for each attenuation estimate. \revi{avg_1}{Fading does not give information on the \ac{ue} location, thus it is a disturbance for \ac{irlv}. However by performing $k_f$ estimates of the attenuation in a short time averaging them we obtain the new attenuation estimate $a^{(n)}_{\Sigma} = \frac{1}{k_f}\sum_{j=1}^{k_f} a^{(n)}$.}
...
\revi{lambda}{Parameter $\Lambda$ must be chosen to obtain a desired significance level, \ie a desired \ac{fa} probability. It can be set either by assessing the \ac{fa} probability through simulations or by inverting the expression of \ac{fa} probability as a function of $\Lambda$, when available \protect{\cite[Section 3.3]{Kay-book}}.}
...
\begin{figure}
    ...
    \label{fig:simpScen}
\end{figure}
\revi{simpleScen}{As example of application of \ac{np} test we consider a simple scenario wherein area $\mathcal{A}$ is a circle with radius $R_{\rm out}$ and \ac{roi} $\mathcal{A}_{0}$ is a ring concentric to the overall area, with larger radius $R_{\rm in}$ and smaller radius $R_{\rm min}$. A single \ac{ap} ($N_{\rm AP} =1$) is located at the \ac{roi} center and a \ac{ue} is transmitting from distance $d$. We consider two models: a) \emph{uncorrelated fading scenario}, which includes path-loss and fading, and b) \emph{uncorrelated shadowing scenario}, which includes path loss and spatially uncorrelated shadowing. In both cases we consider the \ac{los} model for path-loss.}
...
p(a|\mathcal{H}_0) = \int_{0}^{R_{\rm in}} p\left( a | d = d_0\right)p(d_0| d_0 \in \mathcal{A}_0) \, {\tt d}d_0,    
\end{equation}
\revi{simpleScen2_1}{where $p(d_0| d_0 \in \mathcal{A}_0)$ is the \ac{pdf} of the \ac{ue} transmitting from distance $d_0$ inside the \ac{roi}. Assuming that \ac{ue} position is uniformly distributed in $\mathcal{A}$ we have, let $\Delta_0 = R_{\rm in}^2-R_{\rm min}^2$ and $\Delta_1= R_{\rm out}^2-R_{\rm in}^2$ $p(d_0| d_0 \in \mathcal{A}_0) = \frac{2 d_0}{\Delta_0}$ for $d_0 \in [R_{\rm min}, R_{\rm in}]$ and $p(d_0| d_0 \in \mathcal{A}_0) = 0$ otherwise.}
\revi{simpleScen2_2}{A similar expression hold for $p(d_0|d_0 \in \mathcal{A}_1)$.} Closed-form expressions of the  \ac{llr} in the two scenarios are derived in Appendix \ref{sec:llrDer}.

\revi{llrComp}{However, in general (e.g., with either multiple \acp{ap} or correlated shadowing/fading) the \ac{llr} can not be computed in closed-form, thus making  \ac{np} test problematic.}

\subsection{Estimated Distance Approach} \label{seccomp}
\revi{literature_1}{We will compare our \ac{irlv} solutions with the \acl{eda} of {\cite{li2010security}}. In \ac{eda}, first the estimate $\hat{L}(x_{\rm UD},x^{n}_{\rm AP})$ of the \ac{ue}-\ac{ap} distance is obtained by inverting the path-loss formula, and then, the \ac{ue} position is estimated by solving 
}
\begin{align}
 ...
 \label{probmindist}
\end{align}
\revi{literature_2}{Let $\mathcal B_0$ be the set of points of the border of $\mathcal A_0$, let the estimated distance of the \ac{ue} from the  border $\mathcal{B}_0$ $d = \min_{\bm{x} \in \mathcal{B}_0} \pm||\hat{\bm x}_{\rm UD} - \bm{x}||$,  where the sign is negative if $\hat{\bm x}_{\rm UD} \in \mathcal A_0$, and positive otherwise. Lastly, $d$ is compared with a suitable threshold $d_0$, chosen in order to achieve a desired FA probability, $\hat{\mathcal{H}}_{\rm MMSE} = \mathcal{H}_0 \quad \text{if } d < d_0$, $\hat{\mathcal{H}}_{\rm MMSE} = \mathcal{H}_1 \quad {\rm otherwise}$.} \revi{literature_3}{Note  this approach requires the knowledge of the path-loss model (including knowledge of LOS and non-LOS state), which is quite unrealistic. Moreover, the estimator (\ref{probmindist}) is not  optimal, since the position error is not Gaussian.}
...
%
Each layer $\ell=0, \ldots, Q-1$, has $N^{(\ell)}$ outputs obtained by processing the inputs with $N^{(\ell-1)}$ functions named neurons. The output of the $n^{\rm th}$ neuron of the layer $\ell$ is
$
y_n^{(\ell+1)} = \psi^{(\ell)}\left( \bm{w}_n^{(\ell)}\bm{y}^{(\ell)}+b_n^{(\ell)} \right),
$
where the mapping between the input and the outputs is given by the {\em activation function} $\psi^{(\ell)}(\cdot)$. The argument of the activation function is a weighted linear combination, with (row vector) weights $\bm{w}_n^{(\ell)}$, of the outputs $\bm{y}^{(\ell)}$ of the previous layer plus a bias $b_n^{(\ell)}$. We focus here on feedforward \acp{nn}, i.e., without loops between neurons' input and output, an architecture also known as \ac{mlp}. For a in-depth description of \acp{nn} refer for example to \underline{\cite[Chapter 6]{goodfellow}}.\revi{hyper1}{Activation functions are typically chosen before training, while vectors $\bm{w}_n^{(\ell)}$ are adapted according to the perceptron learning algorithm in order to optimize the desired output.}

In our setting, the input of the \ac{nn} is the attenuation vector $\bm{a}$, $N=N_{\rm AP}$, and the output layer has a single neuron ($O=1$) providing as output the scalar $y^{(Q)}_1$. Let $\tilde{t}(\bm{a}) = y^{(Q-1)}_1$ be the output of the \ac{nn} corresponding to the attenuation vector input $\bm{a}$. A threshold $\lambda$ is used on the \ac{nn} output to obtain the test function
\begin{equation}
\label{testfunNN}
    f(\bm{a}) = \begin{cases}
...
\revi{lambdaNN}{Parameter $\lambda$ shall be chosen in order to obtain the required \ac{fa} probability. The value of $\lambda$ which guarantees a certain \ac{fa} probability can obtained by simulation, whereas it can not be obtained by inverting the \ac{fa} probability function as could be done in the \ac{np} framework for $\Lambda$. This is due to the fact that the \ac{ml} framework is applied when there is no knowledge of the distribution of the variables and hence we can not compute a closed form solution for the \ac{fa} probability.} \footnote{Notice that usually, for zero-one loss function, literature assumes $\lambda = 0.5$ in order to perform classification. However this does not allow to control neithe \ac{fa} or \ac{md} probabilities and hence build the \ac{det} curve.}

\revi{ceNeeded}{Based on the loss function to be optimized during training \acp{nn} can solve different problems. Based on our purpose in exploiting \acp{nn} we here consider two widely used loss functions: \ac{mse} and \ac{ce}. }
...
\label{teoruck}
\end{theorem}
\revi{teoruck2}{Hence Theorem \ref{teoruck} proves that in the presence of perfect training an infinite number of neurons and at convergence of the training phase to the minimum error, the function implemented by \ac{mlp} is the Bayes optimal discriminant function. Now we have the following corollary.}
\begin{corollary}
\label{th:nn_np}
The test function (\ref{testfunNN}) where the \ac{mlp} is designed according to the \ac{mse} criterion with an infinite set of training points ($S \rightarrow \infty$) and convergence of \ac{mlp} parameters updates to the minimum \ac{mse} converges to a test function equivalent to the \ac{np} test, thus it is the most powerful test.
\end{corollary}
\begin{proof}
...
\begin{theorem}
\label{th:nn_np2}
The test function (\ref{testfunNN}) where the \ac{mlp} is designed according to the  \ac{ce} criterion with an infinite set of training points ($S \rightarrow \infty$) and at convergence of the \ac{mlp} parameter updates to the minimum \ac{ce} converges to a test function equivalent to the \ac{np} test, thus it is the most powerful test.
\end{theorem}
\begin{proof}
...
$
When training is performed with a \ac{ce} loss function, the output of the \ac{mlp} is the minimum \ac{mse} approximation of the probability $\mathbb{P}(\mathcal{H}_0|\bm{a}^{(i)})$ of being in hypothesis $\mathcal{H}_0$ given the attenuation vector $\bm{a}$ \underline{\cite[Section~5.2]{Bishop2006}}, i.e.,
$
...
\begin{subequations}
	...
	\end{equation}
\end{subequations}
\revi{hyper3}{where $C$, together with the kernel scale {\cite[Chapter 6]{Bishop2006}}, is not optimized by the learning algorithm, but can be tuned on training data using a separate procedure, e.g., see \cite{guo2008novel}.}
...
\revi{revOC}{In practice, having training points available for both regions $\mathcal A_0$ and ${\mathcal A}_1$ may be difficult since region $\A{1}$ may be large and not necessarily well defined (being simply the complementary region of $\mathcal A_0$), and it may be complicated to collect significant training samples from this area.} Therefore, we consider in this section methods that rely only on the statistics and data collected from within the region $\mathcal A_0$. \revi{oneClass}{During the training phase we collect from within area $\mathcal{A}_0$ attenuation vectors that are used to train a \ac{ml} classifier to distinguish vectors belonging to $\mathcal{A}_0$ and $\mathcal{A}_1$ in the testing phase.} This problem can also be denoted as one-class classification, since we know only samples taken from one of the two classes of the problem. 

With reference to the \ac{np} approach, when only the channel statistics from within $\mathcal A_0$ are known a-priori, the \ac{llr} \eqref{eq:lr} can not be used as discriminant function, as $p(\bm{a}|\mathcal{H}_1)$ is not known. In this case, we can resort to the \ac{glrt} \cite{Kay-book}, which, although in general sub-optimal, is a meaningful generalization of the \ac{np} test, providing the test function becomes 
...
\revi{deep ae}{We consider the \ac{ae} \protect{\cite{Hinton-2006}}, i.e., a \ac{nn} that is trained to attempt to copy its input to its output. It comprises an encoder NN (with $N_e$ layers), which transforms the high dimensional input data into a low-dimensional code and a decoder NN (with $N_d$ layers), which reconstructs the original data from the low-dimensional code. For an in-depth description of the \ac{ae} architecture and the representational meaning of the hidden layer values please refer to \protect{\cite[Chapter 14]{goodfellow}}. 
...
For our \ac{irlv} problem, we train the \ac{ae} with attenuation vectors $\bm{a}^{(i)}$ taken only when the trusted \ac{ue} is in  \ac{roi} $\mathcal A_0$. Then, by letting $\bm{y}^{(L)}(\bf a)$ be the output vector of the \ac{ae} for the attenuation input $\bm{a}$, the \ac{mse} of the reconstruction is 
...
f(\bm{a}) =
\begin{cases}
1 &\text{if } \epsilon(\bm{a} ) \geq \lambda^{(\rm AE)}, \\
-1 & \text{if } \epsilon(\bm{a} ) < \lambda^{(\rm AE)},
\end{cases}
\end{equation}
where again $\lambda^{(\rm AE)}$ must be chosen to achieve a desired \ac{fa} probability.

\revi{mseThresholding}{As the \ac{ae} attempts to copy the input to its output, it turns out that, in the testing phase, only vectors with features similar to those of the training set will be reconstructed with small \ac{mse}, whereas input vectors with different features will be mapped to different vectors at the output, with high \ac{mse}. Since training is based on vectors collected from area $\mathcal{A}_0$ and we want to verify users located inside $\mathcal{A}_0$, by thresholding the \ac{mse} we can obtain the classifier needed for the \ac{irlv}.}
...
\uline{In this section, we present the performance of the proposed \ac{irlv} obtained from both experimental data and the channel models of Section~II. We consider a unitary transmitting power for each user and a carrier frequency of $f_0=2.12$~GHz, unless differently specified. When spatial correlation of shadowing is assumed, we consider a correlation distance of $d_c = 75$~m according to the model of Section~II.} \uline{The training points for the classification tasks are taken uniformly over the area $\mathcal A$ ($\mathcal A_0$).}
 
\revi{activation}{For the \ac{nn} approach, we used fully connected networks. For the two-class classification problem, the activation function of the input layer is the identity function, while the activation function of the  hidden and output layers is the sigmoid function \protect{\cite[Section 6.2.2.2]{goodfellow}}}. \revi{numResSimplScen3}{All results for \ac{nn} have been reported only for \ac{ce} loss function, as we have shown in Corollary 1 and Theorem 2 that with both loss functions we achieve the same performance of the \ac{np} test.}

\subsection{Two-class \ac{irlv} With Single \ac{ap}}\label{sec:singleAp}

We start with a \ac{irlv} system using a single \ac{ap}. We consider the two channel models in Section~\ref{sec:los}.
\paragraph*{Uncorrelated fading/shadowing} \revi{numResSimplScen}{Firstly, we consider the environment of Section \ref{sec:los} describing a small area. Channel model includes  spatially uncorrelated fading or shadowing are considered, with $R_{\rm out}= 10$~m, $R_{\rm in} = 2$~m and $R_{\rm min} = 0.1$~m. Moreover, \ac{los} is assumed for path-loss. For uncorrelated fading, we consider two path-loss coefficients, namely  $\nu=2$ and $3$; the closed-form expression of the \acp{llr} for the \ac{np} test are given by (\ref{eq:llr1}) and (\ref{eq:llr2}). With spatially uncorrelated shadowing, we set $\nu=2$, and three values of shadowing variance, namely  $\sigma_{s, {\rm dB}} = 0.1$~dB, $1.8$~dB, and 6~dB; the closed-form expression of the \acp{llr} for the \ac{np} test is given by (\ref{eq:llr3}). For the \ac{ml} approaches, we consider $S=10^5$ training points and a \ac{nn} with $N_L = 2$ hidden layers, each layer with $N_h=5$ neurons and the \ac{ce} loss function.}
...
\revi{numResSimplScen2}{Fig. \ref{fig:ceVSnp} shows the \ac{fa} probability versus the \ac{md} probability i.e., the  \ac{roc}, obtained with the \ac{np} test,  the \ac{nn} and \ac{lssvm} classifiers. We notice that all models achieve the same performance, confirming our theoretical results that both \ac{nn} and \ac{lssvm} with sufficient training data and number of hidden layers are as optimal as \ac{np}.} \revi{numResSimplScen4}{We observe that fading has more impact on the performance than shadowing, yielding higher \ac{fa} and \ac{md} probabilities. Still, with fading, a higher path-loss coefficient provides better results as the attenuation increases more with the distance, thus easing classification. For spatially uncorrelated shadowing, performance improves as $\sigma_{s, {\rm dB}}$, since in this case path-loss alone already allows to take error-free decision, thus the shadowing component is a disturbance in the decision process.}
...
\uline{We now consider the spatially correlated shadowing  ($\sigma_{s, {\rm dB}} = 8$~dB) of Section~II. The considered environment is shown in Fig. \ref{fig:mBS} with only $\rm AP_6$  located at the street intersection and a square \ac{roi} with $d_1= 50$~m, $d_2= 50$~m and $\beta_1 = \beta_2 = 150$~m.} \revi{building}{The \ac{roi} is inside the  south-west building, modelling for example a scenario wherein privileged network resources are accessible only to users inside an office.} Along the streets, \ac{los} propagation conditions hold (with $\nu = 2$), while no-\ac{los} propagation conditions hold in the rest of the area. 
...
\paragraph{Effect of fading average}  \uline{As discussed in Section~\ref{sec:chMod}, for a given \ac{ue} position, the attenuation changes over time due to fading. By averaging $k_f$ realizations of the attenuation in the same position, the effect of fading on \ac{irlv} is mitigated.}  

We consider the environment of Fig. \ref{fig:mBS} with $N_{AP} =5$ \acp{ap}, \uline{i.e., ${\rm AP}_i$ with $i = 1,\dots,5$}. The values of the channel parameters are those of Fig. \ref{fig:kf1}. For $n_x$  explored locations by the \ac{ue}, and $k_f$  fading realizations per location, we obtain $S= n_x \cdot k_f$ training attenuation vectors. Fig. \ref{fig:kf10-5}  shows the \ac{roc} for $k_f=1$ and 10, \uline{with $n_x = 2 \cdot 10^4$ for \ac{svm} and $n_x = 3.2 \cdot 10^5$ for \ac{nn}. } \revi{revLI2a}{We also report the performance of both \ac{eda} and of \ac{ml} methods and for \ac{eda} we also assumed to know the relation between the attenuation and the distance.} \revi{rev2fad}{We note that both \ac{md} and \ac{fa} probabilities can be significantly reduced by averaging fading, thus approaching the performance of \ac{irlv} operating on channels without fading. Indeed, an average of 10 fading realizations already reduces the average \ac{md} probability from $10^{-1}$ to $10^{-2}$, for an \ac{fa} probability of $2\cdot 10^{-1}$, while we achieve an average \ac{md} probability of $4\cdot 10^{-4}$ without fading using a \ac{nn}. We also notice that, in absence of fading, \ac{svm} significantly outperforms  \ac{nn} even if \ac{nn} uses a larger $S$. This suggests that, in this scenario, the \ac{nn} has not yet converged to the optimum, wherein potentially very good performance can be achieved, due to limits in architecture, computational capabilities, and design algorithms. We should remember, in fact, that the number of parameters defining the \ac{svm} grows with the training size, while the number of parameters of the  \ac{nn} is set a-priori.} \revi{revLI2b}{Lastly, we observe that the proposed \ac{ml} techniques (both with and without fading) outperform \ac{eda} (the performance of which has been obtained on channels without fading). This is due to the fact that \ac{eda} is more severely affected by shadowing which is considered as noise in the derivation of the distance, while \ac{ml} solutions may exploit also shadowing in making the decision, while still not relying on specific channel models.} 
...
\paragraph{Results on experimental data} \revi{Berlin}{We have tested the proposed \ac{irlv} solutions on real data collected by the MOMENTUM project {\cite{MOMENTUM-D53}} in a measurement campaign at Alexanderplatz in Berlin (Germany). Attenuations at the \ac{gsm} frequency (that may refer to a cellular IoT scenario in our \ac{irlv} context) have been measured for several \acp{ap} in an area of $4500~{\rm m} \cdot 4500~{\rm m}$ with a measurement grid of $50 \cdot 50$~m. We have considered 10 attenuation maps, corresponding to 10 \acp{ap} located at the locations $\bm{x}_{\rm AP}^{(1)} = [2500, 2500]$, $\bm{x}_{\rm AP}^{(2)} = [500, 4000]$, $\bm{x}_{\rm AP}^{(3)} = [4000, 4000]$, $\bm{x}_{\rm AP}^{(4)} = [500, 500]$, $\bm{x}_{\rm AP}^{(5)} = [4000, 500]$, $\bm{x}_{\rm AP}^{(6)} = [100, 4500]$, $\bm{x}_{\rm AP}^{(7)} = [1000, 400]$, $\bm{x}_{\rm AP}^{(8)} = [4000, 500]$, $\bm{x}_{\rm AP}^{(9)} = [4300, 4000]$, $\bm{x}_{\rm AP}^{(10)} = [4500, 500]$. The \ac{roi}  has been positioned in the lower-right corner corresponding to, following the same notation of Fig \ref{fig:mBS}, $d_1 = 3000$, $ d_2 = 1500$~m, and $\beta_1 = \beta_2 = 1000 $~m.  In this case, we have a single realization of any channel effect (path-loss, shadowing, fading, \ldots) per location for a total of 8464 realizations, 5000 of which have been used for training and the rest for testing. For \ac{nn}, we set $L = 3$ and $N_h = 500$. Fig. \ref{fig:Berlinnew} shows the \ac{roc} for both \ac{nn} and \ac{lssvm}. The  performance is in line with the other figures obtained by simulation. Still, due to the small size of the available training set, \acp{roc} are not smooth. Moreover, we notice that \ac{svm} and \ac{nn} achieve approximately the same performance.}\revi{revLI}{Note also that, in order to use \ac{eda}, we should first know the path-loss model to be used to convert the attenuation estimates into distances, information not immediately available from the experiments. Therefore we could not compare \ac{ml} with \ac{eda} in this case, further demonstrating the utility of \ac{ml} model-less techniques for \ac{irlv}.}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} for one-class \acp{irlv} in a scenario with $N_{\rm AP}=10$, path-loss, shadowing and fading with different training set size and $k_f=1$ fading realizations,  \ac{ae} with $N_L = 7$. }
    \label{fig:kf1Oc}
\end{figure}
...
We now focus on the one-class \ac{irlv} solutions, described in Section~\ref{sec:OneClass}, where the training points come only from the \ac{roi} $\mathcal A_0$. \revi{designAE}{The \ac{ae} has been designed according to  \cite{Hinton-2006}, i.e. all neurons use the logistic sigmoid as activation function except for the central hidden layer where linear activation functions are used. The \ac{ae} has $N_L = 7$ hidden layers with 7, 6, 3, 2, 3, 6, and 7 neurons, respectively. Weights are initialized randomly.} \uline{The channel model is that described in Section~II, the environment is that of Fig. \ref{fig:mBS} (with $N_{\rm AP} =10$), with the parameters set in  Section~\ref{sec:res_fading} and $d_1=50$~m , $d_2=50$~m, and $\beta_1 = \beta_2 = 150$~m.} 
...
\revi{simpleScen3}{Assuming spatially uncorrelated Rayleigh fading, without shadowing (\ie $\sigma_{s,dB}=0)$, given a \ac{ue} located at distance $d$, the channel gain $\sqrt{g}\sim N(0,10^{-\log_{10} P_{\rm PL,LOS}(d)})$. Thus $g=1/a$ is exponentially distributed with mean (in dB) $P_{\rm PL,LOS}(d)$ given by $\ref{eq:los}$. Letting}
\begin{equation}
    F(\Delta,R_0,R_1) = 
    \frac{2}{\Delta}\int_{R_0}^{R_1} 10^{P_{{\rm PL},{\rm LOS}}(d_0)/10} \exp\left(-10^{P_{{\rm PL},{\rm LOS}}(d_0)/10} \frac{1}{a}\right)d_0 \, {\tt d}d_0,
\end{equation}
\revi{simpleScen3_1}{ from the uniform \ac{ue} distribution we have $p(a|\mathcal{H}_0)=F(\Delta_0,R_{\rm min},R_{\rm in})$, whereas  $p(a|\mathcal{H}_1) = F(\Delta_1,R_{\rm in},R_{\rm out})$.}
...
\end{equation}
\revi{simpleScen4_3}{where $\erf(x)= \frac{2}{\sqrt{\pi}}\int_0^x e^{-t^2} dt$ is the error function and}
\begin{equation}
    \mathcal{T}(d_0) = \erf\left( \frac{\frac{100 \nu^2}{\sigma_s^2}\ln d_0-\ln^2(10)+\frac{\frac{1}{a} 10 \nu \ln 10}{2\sigma_s^2}}{\sqrt{1/2\sigma_s^2}10\nu\ln 10}\right),
\end{equation}
...
\begin{itemize}
    \item including a comparison with existing technique that estimates the \ac{ue} position in order to perform \ac{irlv} (see Section~\ref{seccomp} and Fig.s \ref{fig:kf10-5} and \ref{fig:kf10Oc});
...
\begin{framed}
\noindent {\bf Rev:
Neural networks in Figure 2 [figure removed in the new version] are fully connected and kind of shallow.}
\end{framed}
...
\setstretch{1.5}
{\bf Ans:} Thanks for your comment. Following the advice of another reviewer and due to space limitation, we decided to remove the part on attacks from the revised manuscript. We have revised the abstract accordingly.
...
\begin{itemize}
    ...
    \item Other works deal with the problem of a {\em specific} location verification, i.e., checking if the user is in a given position. In this case, the papers in the literature suggest to i) convert attenuation measurements into distances with respect to reference points, ii) determine the user position, and iii) verify it. This approach can be extended to \ac{irlv}, and, in fact, we have included a comparison of this approach with our approaches in the revised manuscript (see  Section~\ref{seccomp}, and the response to your next question). However, this approach require the a-priori knowledge of the relation between the measured physical quantity (e.g., the attenuation) and the distance. With realistic channel models, this relation is not fixed but depends on the position itself (e.g., depending on line of sight, non line of sight conditions); therefore, this approach is not robust, as also shown by our results.
...
\setstretch{1.5}
{\bf Ans:} Thank you for the suggestion. In the revised manuscript, we have included the description of the method presented in \cite{li2010security}, wherein the position of the user is first estimated and then compared with the area of interest. Still, the technique of \cite{li2010security} a) assumes to know the relation between path-loss and distance, which may be unrealistic, and b) is designed to confirm a specific position rather than asserting if the user is in a {\it region}. Therefore, we have extended \cite{li2010security} to perform \ac{irlv}. The resulting method is briefly described in Section~\ref{seccomp} as follows:
\begin{quote}
\hl{***}    "\revp{literature_1} 
	\begin{align}
 \hat{\bm x}_{\rm UD} =  \underset{\bm x}{\arg \min} \sum_{n=1}^{N_{\rm AP}} \left(L(\bm{x},\bm{x}_{\rm AP}^{(n)}) - \hat{L}(\bm{x}_{\rm UD},\bm{x}_{\rm AP}^{(n)}) \right)^2,.
\end{align}
\revp{literature_2}
...
Moreover, Fig.s  \ref{fig:kf10-5} now include the comparison with the model in the literature. We have added the following comments on the figures:  
\begin{itemize}
    ...
    \begin{quote}
        "\revp{revLI3}"
    \end{quote}
    \item Moreover, we have added the following comment to Fig. \ref{fig:Berlinnew}:
    \begin{quote}
        "\revp{revLI}"
    \end{quote}
\end{itemize}
...
\begin{enumerate}
    ...
    \item we have presented results (without comparison with the \ac{np} test) for channels  including a) path loss with LOS/noLOS characteristics, b) spatially correlated shadowing, and c) fading, i.e., in accordance with the most detailed models in the literature. 
\end{enumerate}
In particular, about point 1), we have revised the entire Section~\ref{sec:los} in order to include more realistic scenarios and show the effect of different channel parameters on the \ac{irlv} performance. In Section~\ref{sec:los}, we now consider the following scenarios:
\begin{quote}
"\revp{simpleScen}"
\end{quote}
Then, we derive closed-form expressions of the \ac{llr} in both scenarios.

In the Numerical Result section, performance results are reported for the new scenarios, confirming that both \acp{nn} and \ac{svm} match the performance of the \ac{np} test. In particular, we write:
\begin{quote}
...
\setstretch{1.5}
{\bf Ans:} The proposed \ac{ml} solutions operate with any channel feature, and the comparative analysis on their asymptotic behaviour with respect to \ac{np} test holds irrespective of the input model. Moreover, the \ac{ml} approach is model-less by nature, as no assumption is done on the structure/model of the input. In this sense, the details provided for the channel model in Section~II only show the complexity of the \ac{irlv} problem and are used to obtain numerical results in real conditions. However, we do not use the specific model of Section~II for the analysis described in the rest of the paper. In order to strengthen the fact that our approach is general and not related to a specific model, we report performance results obtained by  applying the \ac{ml} \ac{irlv} techniques also on experimental data in the revised manuscript.

In order to make this point more more clear:
\begin{itemize}
...
\begin{quote}
    "\revp{WiFi2}"
\end{quote}
Lastly, most numerical results are obtained from a 2~GHz frequency that well fits WLAN applications, while the newly inserted results from experimental data are referring to GSM frequencies, just as an example of a cellular IoT.
...
\setstretch{1.5}
{\bf Ans:} Thank you for pointing out this. We have clarified this point and added a reference to the standard \ac{np} test design. We have added the following sentence in Section~\ref{sec:auth}
\begin{quote}
"\revp{lambda}"
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} In order to provide more realistic scenarios, we have replaced the example you mentioned in the new manuscript with two new scenarios where the channel is affected by path-loss, fading and uncorrelated shadowing (see the answer to one of your previous questions). Therefore, parameter $\zeta(\rho)$ does not exist anymore. 
...
\setstretch{1.5}
{\bf Ans:} We decided not to include the plot for the test functions (\ref{eq:llr1}), (\ref{eq:llr2}) and (\ref{eq:llr3}), as we already have many figures and they do not provide any additional insight to the problem. As these are some of the few cases where the \ac{llr} has a closed form solution, the insight given by this example is the numerical proof of what stated by Corollary 1 and Theorem 2, \ie hypothesis testing performed via \ac{ml} is optimal in the \ac{np} sense. In order to clarify this, we added in Section~\ref{sec:los}:
\begin{quote}
"\revp{llrComp}"
\end{quote}

Moreover, in Section \ref{sec:singleAp}, we have added the following sentence:

\begin{quote}
    "\revp{numResSimplScen2}".
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} The parameter $\lambda$ must be chosen to guarantee the desired $P_{\rm FA}$. In order to clarify this point, we have added the following sentence in Section~\ref{sec:nn}:
\begin{quote}
"\revp{lambdaNN}"
\end{quote}
...
\end{framed}
\setstretch{1.5}
The \ac{nn} can be designed according to many target functions. We have considered the widely used \ac{mse} and \ac{ce} functions and shown with two different theorems that the \ac{nn} achieves performance of the \ac{np} test with both choices. 
...
\end{framed}
\setstretch{1.5}
We have merged Sections D and E on \ac{svm}, as you suggested.
...
\begin{framed}
\noindent {\bf Rev:
The phrase "parameter" is first used explicitly in Sec. 3-E without introducing it. However, it is implicitly used earlier in the manuscript when the activation function in NNs and kernel function in SVMs are described as being "fixed" (which refers to being a hyperparameter).}
\end{framed}

\setstretch{1.5}
{\bf Ans}: We thank the reviewer for pointing out the issue. We dropped the adjective {\it fixed} in favour of clearer statements in Section~\ref{sec:nn} and \ref{sec:svm} which now read:
\begin{quote}
	"\revp{hyper1}"
\end{quote}
and
\begin{quote}
	"\revp{hyper2}"
\end{quote}
Then, we also dropped the term {\it hyper-parameter} substituting it directly with its definition. We modified another paragraph in Section \ref{sec:svm} which now reads
 \begin{quote}
 	"\revp{hyper3}"
 \end{quote} 
...
\setstretch{1.5}
{\bf Ans:} In Section~\ref{sec:OneClass} the new model collects data uniquely from region $\mathcal{A}_0$ in the training phase and consider \ac{ml} classifiers able to distinguish between two classes while being trained only with samples from one class. Hence, samples are collected from $\mathcal{A}_0$ during the training phase and from both $\mathcal{A}_0$ and $\mathcal{A}_1$ during the testing phase. The reason for this choice is that the area $\mathcal{A}_1$ can be large or not well defined, and it may be complicated to collect significant training samples from this area. Therefore a simple approach with a smaller known set is of interest. 
In order to clarify this aspect we added in Section~\ref{sec:OneClass}:
\begin{quote}
"\revp{oneClass}"
\end{quote}
...
\end{framed}
\setstretch{1.5}
{\bf Ans:} According to your suggestion and due to space limitation, we removed this subsection on ML-based attacks, and associated numerical results.
...
{\bf Ans:} 
Thanks for having highlighted this issue. We have carefully revised both \ac{mlp} and AE design choices and we now use common activation functions from the literature. In particular, in Section~\ref{sec:numRes} we write
\begin{quote}
    "\revp{activation}"    
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} Thank you for your note. Indeed, we have revised our approach in the presence of fading, and we now average the $k_f$ fading realizations and give the resulting average as input to the learning machines. In particular we described the fading average in Section~\ref{sec:auth} as:
\begin{quote}
"\revp{avg_1}
\end{quote}
...
\setstretch{1.5}
{\bf Ans:} Thank you for the suggestion. In the revised manuscript we have included the description of the  method presented in \cite{li2010security}, wherein first the position of the user is estimated and then compared with the area of interest. Still, the technique of \cite{li2010security} a) assumes to know the relation between path-loss and distance, which may be unrealistic, and b) was designed to confirm a specific position rather than asserting if the user is in a {\it region}. Therefore, we extended of \cite{li2010security} to perform \ac{irlv}. The resulting method is briefly described in Section~\ref{seccomp} as follows:
\begin{quote}
\hl{***}    "\revp{literature_1} 
	\begin{align}
 \hat{\bm x}_{\rm UD} =  \underset{\bm x}{\arg \min} \sum_{n=1}^{N_{\rm AP}} \left(L(\bm{x},\bm{x}_{\rm AP}^{(n)}) - \hat{L}(\bm{x}_{\rm UD},\bm{x}_{\rm AP}^{(n)}) \right)^2,.
...
\revp{literature_3}"
\end{quote}
Moreover, Fig.s  \ref{fig:kf10-5}, and \ref{fig:kf10Oc} now include the comparison with the literature model. We have added the following comments on the figures:  
\begin{itemize}
    \item Comment to Fig. \ref{fig:kf10-5} :
    \begin{quote}
        "\revp{revLI2a}" \\ ... \\ "\revp{revLI2b}" 
    ...
    \begin{quote}
        "\revp{revLI3}"
    \end{quote}
    \item Moreover, we added the following comment to Fig. \ref{fig:Berlinnew}:
    \begin{quote}
        "\revp{revLI}"
    \end{quote}
\end{itemize}
...
\setstretch{1.5}
{\bf Ans:} We added Section~\ref{sec:comp} showing the computational complexity of the two machine learning methods. The section reads as follows:
\begin{quote}
"\revp{comp1} 
...
\setstretch{1.5}
{\bf Ans:} Based on your comments we revised Fig. \ref{fig:ceVSnp} and the relative theoretical part in Section~\ref{sec:los}. The new model also includes uncorrelated shadowing and fading effects. Results show that the implemented system achieves better performance and the equivalence between hypothesis testing based on the \ac{np} framework and via \ac{ml} is still verified. Furthermore we improved the performance of the overall system when considering correlated and shadowing effects for both two-classes and one-class classifiers by adding more layers to the \ac{nn} and considering a larger number of \acp{ap}.
