...
We first proceed as in the proof of Theorem \ref{th:lsnp} and re-write problem \eqref{eq:oneClassSvm} as
	\begin{subequations}
		...
		\begin{equation}
		\label{eq:oclssvm2}
		\underset{\mathbf{w},e}{\text{min}} \quad f_0' = \frac{1}{2} \mathbf{w}^T \mathbf{w} + C S \frac{1}{2} \sum_{j=1}^M p_{\bm{a}^{(i)},t_i}(\bm{\alpha}_j,1) e_j^2 - \rho  
		\end{equation}
		\begin{equation}
		\label{eq:ocstpart2}
...
\begin{figure}
    ...
    \caption{Convergence of \ac{mlp}. $P_{\rm MD}$ vs. $P_{\rm FA}$ for \ac{mlp} with $N_h = 6$ and with different training set size.}
    \label{fig:faded}
\end{figure}
...
\begin{figure}
    ...
    \caption{Convergence of \ac{svm}. $P_{\rm MD}$ vs. $P_{\rm FA}$ for \ac{svm} with different training set size.}
    \label{fig:faded}
\end{figure}
...
\section{Numerical Results: two class classification}
