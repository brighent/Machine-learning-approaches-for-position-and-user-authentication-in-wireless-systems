...
\label{sec:comp}
\revi{comp1}{
In this section, we briefly review the computational cost for i) training each model and ii) make a prediction on a new data point. The main elements of this analysis are:}
\begin{itemize}
\item $S$ training points;
\item $N$ input features; % for both \ac{nn} and \ac{svm};
\item $N_L$ number of hidden layers of a \ac{nn};
\item $epochs$ number of epochs (how many times each training point will be used) of a \ac{nn};
\item $N_{SV}$ number of support vectors of a \ac{svm}.
\end{itemize}
\revi{comp2}{
For a basic fully connected feed-forward network, the backpropagation training algorithm is $O(epochs \cdot S \cdot N_L \cdot N^3)$ when the number of hidden nodes of each hidden layer is proportional to the size of the input, while the prediction of a new unseen data point is $O(N_L \cdot N^3)$. For a more detailed analysis which takes into account also the cost of the choice of the  activation function see {\cite{Bianchini2014}}.}

\revi{comp3}{
For a \ac{svm}, the estimate of the vector $\bm{w}$ at training time requires the solution of a quadratic problem the complexity of which is between $O(S^2)$ and $O(S^3)$. At test time, the prediction is linear in the number of features and the number of support vector $O(N \cdot N_{SV})$. Indeed, more efficient online training algorithms are available {\cite{BordesEtAl2005}}.}
...
In this section, we present the numerical results obtained using the proposed \ac{irlv} and attack solutions for the scenario described in Section II. (\hl{Qui inserirei un breve riassunto dell'experimental setup, non solo per le NN. Giorgio}) \revi{activation}{In particular, for the \ac{nn} for two-class classification, the activation function for the input layer is the identity function and for the hidden and output layers is the sigmoid function}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{svm} and \ac{nn} for $N_{\rm AP}=10$ (newArea) and for different numbers of training set size $S$.}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{svm} and \ac{nn} for $N_{\rm AP}=10$ and for different numbers of collected locations ($n_x$) and $k_f=10$ fading realization per location.}
    \label{fig:kf10}
\end{figure}
...
We now focus on the one-class \ac{irlv} solutions, where the training points come only from the \ac{roi} $\mathcal A_0$. \revi{designAE}{The \ac{ae} has been implemented by considering the activation functions in \cite{Hinton-2006}. All neurons use logistic sigmoid activation function (\ref{eq:sigmoid}) except for the central hidden layer which is linear. It is composed by $N_L = 7$ hidden layers with respectively $\{7,6,3,2,3,6,7\}$ neurons. Furthermore we considered random weights initialization.} 
