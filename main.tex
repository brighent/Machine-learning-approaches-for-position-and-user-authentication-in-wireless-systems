...
\revi{attack1}{
In the location verification literature there are two classes of threats, either the attacker sends forged messages to the verification system \cite{singelee2005location} \cite{song2008secure} \cite{Sastry} or he tampers with the signal power making it coherent with the fake claimed position \cite{Vora} \cite{yan2016location} \cite{li2010security}. The first class of attacks targets verification systems which require bits transmission, while countermeasures for the second class are based on channel features like \ac{rss}. In this paper we focus on the second class, as our protocol does not require the decoding of verification messages or nonces.
}
...
\revi{lit2}{We design our system in a scenario with channel impairments such us shadowing and fading, which is new in the location verification literature. Indeed, in {\cite{Vora}} \ac{rss} measurements are explicitly treated interchangeably as distance measures neglecting all shadowing and fading phenomena. Similarly, in {\cite{Sastry}} it is generically assumed that agents are able to communicate and exchange packets without errors, as in all protocols requiring nonces transmission {\cite{singelee2005location}}, {\cite{song2008secure}}. In {\cite{yan2016location}} and {\cite{li2010security}} shadowing is taken into account, even though not fading. However, in {\cite{yan2016location}} shadowing is relative to a single user position, allowing the authors to derive the channel statistic and in {\cite{li2010security}} authors assume the knowledge of the path-loss model. Both these hypotheses are not required by our \ac{irlv} system. Moreover, in this paper we focus on \ac{irlv}, which is different from verifying a claimed specific location {\cite{singelee2005location} \cite{li2010security}} or authenticating a user with its location {\cite{yan2016location}}}.
...
\begin{figure}[h]
    ...
    \caption{\ac{roc} of \acp{irlv} methods for \ac{los}, uncorrelated fading/shadowing and various values of $\nu$ and $\sigma_{s, {\rm dB}}$. Environment of Fig. \ref{fig:simpScen}. \colorbox{red}{USARE + PER LSSVM E METTERE IN UNA SOLA FIG}}
    \label{fig:ceVSnp}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{Example of attenuation map including path-loss and shadowing, with the \ac{ap} positioned at the center. \colorbox{red}{QEUSTO é MISTO LOS/noLOS, giusto? TOGLIERE l'AREA $\mathcal A_0$!}}
    \label{fig:map}
\end{figure}

\uline{We now consider the spatially-correlated shadowing  ($\sigma_{s, {\rm dB}} =$\hl{...}), \hl{CONFERMIno-los?} of Section II, and the environment of Fig. \ref{fig:mBS} with only one  \ac{ap}  located at the street intersection, and a square \ac{roi} with $d_1=$\hl{...} and $d_2= $ \hl{...}.} \revi{building}{The \ac{roi} is inside the  south-west building, modelling for example a scenario wherein privileged network resources are accessible only to users inside an office.} Along the streets, \ac{los} propagation conditions hold (with $\nu =$ \hl{...}), while no-\ac{los} propagation conditions hold in the rest of the area. Fig. \ref{fig:map} shows a realization of the  attenuation map (including both path-loss and shadowing), highlighting the different propagation conditions. \uline{Since no close-form expression of the \ac{llr} is available in this scenario, we quantize the attenuations collected in the learning phase  with a large alphabet and estimate the sampled \ac{pdf} for the quantized attenuations. Lastly, we use the estimated \ac{pdf} to compute the \acp{llr}. We use $4.46 \cdot 10^6$ training points in the area $\mathcal A$, and a uniform quantizer for the attenuation (within the observed extreme values) with $300$ quantization values. \colorbox{red}{CONFERMATO? $\rightarrow$}Only $10^3$ points are instead used for training of both \ac{mlp} and \ac{svm}.} 
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{irlv} methods, with a \ac{nn} having $N_L=1$ and two values of $N_h$. Environment of Fig. \ref{fig:mBS}, with one \ac{ap} located at the street intersection, $d_1 = $\hl{...} and $d_2 = $\hl{...}, and correlated shadowing ($\sigma_{s, {\rm dB}} = $ \hl{...}.}
    \label{fig:trueMap}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{irlv} methods for different averages of fading. Environment of Fig. \ref{fig:mBS}, with  $N_{\rm AP}=5$, $d_1 = $\hl{...} and $d_2 = $\hl{...}, $\sigma_{s,{\rm dB}} = $\hl{...}.}
    \label{fig:kf10-5}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{\ac{roc} of \ac{irlv} methods for different averages of fading. Environment of Fig. \ref{fig:mBS}, with  $N_{\rm AP}=10$, $d_1 = $\hl{...} and $d_2 = $\hl{...}, $\sigma_{s,{\rm dB}} = $\hl{...}.}
    \label{fig:kf10}
\end{figure}
...
We consider the environment of Fig. \ref{fig:mBS} with $N_{AP} =5$ \acp{ap}, four at the ends of the streets, and one  at the street intersection. Channel parameters are those of Fig. \ref{fig:kf1}, and $S=$\hl{...}. For $n_x$  explored locations by the \ac{ue}, and $k_f$  fading realizations per location, we obtain $S= n_x \cdot k_f$ training attenuation vectors. Fig. \ref{fig:kf10-5}  shows the \ac{roc} for $k_f=1$ and 10. \revi{revLI2a}{We also report the performance for a channel without fading ({\it no fading} in the legend). Lastly, we show the performance of  \ac{eda}, described in Section \ref{seccomp}, still obtained without fading and assuming to know the relation between the attenuation and the distance.}\revi{rev2fad}{We note that both \ac{md} and \ac{fa} can be significantly reduced by averaging the fading, approaching the no-fading performance. Indeed, an average on 10 fading realizations already reduces the average \ac{md} probability from $10^{-1}$ to $10^{-2}$ for an \ac{md} probability of $2\cdot 10^{-1}$, while without fading we achieve an average \ac{md} probability of $4\cdot 10^{-4}$ (using a \ac{nn}). We also notice that in the absence of fading \ac{svm} significantly outperforms  \ac{nn}, suggesting that in this scenario, wherein potentially very good performance can be achieved, the \ac{nn} has not yet converged to the optimum, for limits in both training and computational capabilities. We should remember in fact that the complexity of the function implemented with the \ac{svm} grows with the training, while complexity of the  \ac{nn} (i.e., the number of its tunable parameters) is fixed.}\revi{revLI2b}{Lastly, we observe that the proposed \ac{ml} techniques (both with and without fading) outperform  \ac{eda}  (whose performance has been obtained on channels without fading). This is due to the fact that \ac{ml} solutions are very robust and do not rely on specific channel models; moreover they do not convert attenuation measures into intermediate quantities obtained by specific models (as instead \ac{eda} does, by converting them first into distances), but directly take a decision directly on the attenuation estimates.} 
...
\uline{We consider now the \ac{ml} attack strategies described in Section \ref{sec:attack} for the scenario of Fig. \ref{fig:mBS}, with $N_{\rm AP} =5$ \acp{ap}, and channels including path-loss, shadowing, and fading as described in Section II \hl{check:} the parameters set in  Section \ref{sec:res_fading} and $d_1=$\hl{...}, $d_2=$\hl{...}.  \ac{irlv} is  performed with one-class classifiers.  \ac{roi} $\mathcal A_0$ coincides with the whole south-west building of Fig. \ref{fig:trueMap}. } 
...
\begin{itemize}
    \item We have added the new references \cite{singelee2005location}, \cite{song2008secure}, and \cite{7398138} to the paper;
...
\begin{itemize}
    ...
    \end{quote}
    \item Comment to Fig. \ref{fig:Berlin}:
    \begin{quote}
        "\revp{revLI}"
    \end{quote}
...
We then applied our proposed \ac{ml} \ac{irlv} technique also on a real dataset, available in \cite{MOMENTUM-D53}. We show the new numerical results in Fig. \ref{fig:Berlin} and added a paragraph in Section \ref{sec:res_fading} which reads:
\begin{quote}
