...
\uline{We consider the environment of Fig. \ref{fig:mBS}, with $N_{\rm AP}=10$ \acp{ap} used for \ac{irlv}, \ie $\rm AP_i$ with $i = 2,\dots,11$. The channel model includes \ac{los} and no-\ac{los} path-loss, spatially-correlated shadowing ($\sigma_{\rm s, Db} = 8$), and fading, as described in Section II. A deep \ac{nn} with $L=3$ hidden layers, each having $N_h = 100$ neurons.}  
...
\uline{We consider now the \ac{ml} attack strategies described in Section \ref{sec:attack} for the scenario of Fig. \ref{fig:mBS}, with $N_{\rm AP} =5$ \acp{ap}, and channels including path-loss, shadowing, and fading as described in Section II the parameters set in  Section \ref{sec:numResOneClass} and $d_1=0$ m, $d_2=0$ m, $\beta_1 = \beta_2 = 255$ m.  \ac{irlv} is  performed with one-class classifiers.  When considering the \ac{ae} we consider an implementation with $N_L = 1$ and $N_h = 3$ in order to maintain a simple structure. \ac{roi} $\mathcal A_0$ coincides with the whole south-west building of Fig. \ref{fig:trueMap}. } 
...
\setstretch{1.5}
{\bf Ans:} The proposed solutions operate with any channel feature, and the analysis about the asymptotic behaviour of \ac{ml} techniques with respect to \ac{np} test hold irrespective of the model of the input. Moreover, note that the \ac{ml} approach is model-less by nature, as no assumption is done on the structure/model of the input. In this sense, the details provided for the channel model in Section II only show the complexity of the \ac{irlv} problem and are used to obtain exemplary numerical results. However, we do not use the specific model of Section II in the rest of the paper. 

In order to make this more more clear we added the following sentences:
\begin{itemize}
    \item \in Section  \ref{sec:intro}:
\begin{quote}
"\revp{framework1}"
\end{quote}
\item in Section \ref{sec:irlvML}: 
\begin{quote}
"\revp{framework2}".
\end{quote}
\end{itemize}

In order to strenghten the fact that our approach is general and not related to a specific model we have applied the \ac{ml} \ac{irlv} technique also on a real dataset, available in \cite{MOMENTUM-D53}. We show the new numerical results in Fig. \ref{fig:Berlinnew} and added a paragraph in Section \ref{sec:res_fading} which reads:
\begin{quote}
